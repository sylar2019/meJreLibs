...
public class ExcelHelper {
    ...
    public ExcelHelper(Workbook workboook) {
        this.workbook = workboook;
    }

    public ExcelHelper(InputStream is, ExcelVersion version) throws FileNotFoundException, IOException {
        if (ExcelVersion.HSS.equals(version)) {
    ...
    public String toString(int sheetIndex) throws IOException {
     ...
     * @throws IOException
     */
    public boolean write(int sheetIndex, List<List<String>> rowData, int startRow) throws IOException {
        Sheet sheet = workbook.getSheetAt(sheetIndex);
        int dataSize = rowData.size();
        if (getRowCount(sheetIndex) > 0) {// 如果小于等于0，则一行都不存在
     ...
     * @throws IOException
     */
    public boolean setStyle(int sheetIndex, int rowIndex, int colIndex, CellStyle style) throws IOException {
        Sheet sheet = workbook.getSheetAt(sheetIndex);
     ...
     * @param lastCol    结束列
     */
    public void mergeCell(int sheetIndex, int firstRow, int lastRow, int firstCol, int lastCol) {
        Sheet sheet = workbook.getSheetAt(sheetIndex);
     ...
     * @throws IOException
     */
    public boolean setValueAt(int sheetIndex, int rowIndex, int colIndex, String value) throws IOException {
        Sheet sheet = workbook.getSheetAt(sheetIndex);
     ...
     * @throws IOException
     */
    public boolean setRowValue(int sheetIndex, List<String> rowData, int rowIndex) throws IOException {
        Sheet sheet = workbook.getSheetAt(sheetIndex);
        Row row = sheet.getRow(rowIndex);
     ...
     * @return
     */
    public List<String> getColumnValue(int sheetIndex, int rowIndex, int colIndex) {
        Sheet sheet = workbook.getSheetAt(sheetIndex);
        List<String> list = new ArrayList<String>();
        for (int i = rowIndex; i < getRowCount(sheetIndex); i++) {
            Row row = sheet.getRow(i);
            if (row == null) {
                list.add(null);
                continue;
     ...
     * @throws IOException
     */
    public boolean setSheetName(int sheetIndex, String name) throws IOException {
     ...
     * @throws IOException
     */
    public boolean removeSheetAt(int sheetIndex) throws IOException {
     ...
     * @throws IOException
     */
    public boolean removeRow(int sheetIndex, int rowIndex) throws IOException {
        Sheet sheet = workbook.getSheetAt(sheetIndex);
        sheet.shiftRows(rowIndex + 1, getRowCount(sheetIndex), -1);
        Row row = sheet.getRow(getRowCount(sheetIndex) - 1);
        sheet.removeRow(row);
        return true;
    }
